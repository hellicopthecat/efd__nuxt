...
export default defineEventHandler(async (event) => {
  ...
  const URL = `${PUBLIC_API_BASE_URL}/DSSP-IF-20590?serviceKey=${config.public.LIFESAFETY_DISASTER_KEY}&pageNo=1&numOfRows=100&safety_cate=${query.id}&returnType=json`;
  try {
    ...
    throw createError({
      ...
      message: "생활안전 행동요령 데이터를 불러오는데 오류가 발생했습니다.",
    });
  }
});
